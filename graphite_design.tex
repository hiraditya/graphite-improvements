...
order to lower its cost in terms of compilation time. Experimental results
on Polybench show that we now detect larger SCoPs 6.09 loops/SCoP as compared
to 2.59 loops/SCoP with the previous algorithm. We also discard programs,
...
optimizer at ``-O3 -fprofile-use'' in the 6.x releases of GCC.
\end{abstract}
...
(the outermost loops, see Section-\ref{subsec:loop-tree}).
...
\begin {figure}
  \begin{center}
    \resizebox{0.8\linewidth}{!}{\input{polybench/speedup}}
  \end{center}
...
\begin {figure}
  \begin{center}
    \resizebox{0.8\linewidth}{!}{\input{gcc/speedup}}
  \end{center}
