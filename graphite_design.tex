...
\begin{enumerate}
\item Discard functions with less than two loops. Since all (?) the optimizations enabled in polyhedral model work on loop nests (e.g., blocking, interchange, tiling etc.).  or on sibling loops (e.g., fusion).
\item Start the scop detection from a CFG node which begins from a loop header. This way the algorithm analyzes statements which matter most. This allows discarding unrepresentable loops early in the scop-detection process.
\end{enumerate}
