...
In GCC a parent loop points to one of its children loops and all other childrens are connected as siblings edges.
Each loop has two pointers `next', which points to one of its sibling loops and, `innner
Suppose, thre are `l' loops in a function ranging from 0 to l-1.
in-degree (loop) = {0, 1}
out-degree (loop) = {0, 1, 2}
loop(l, d)->next = loop(m, d), where l neq m.
loop(l, d)->child = loop(m, d+1)

In the loop tree, only the function has in-degree of 0, all other loops have in-degree of one.
Nodes with out degree of zero represent innermost loops. The sibling loops are connected in program order i.e., loop->next either executes immediately before or immediately after loop with no loops in between.
