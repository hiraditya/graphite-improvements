...
larger extended \SCoP{.}

The following analysis passes are both available in \LLVM{} and \GCC{.}
Graphite and Polly are using these analysis passes to detect \SCoP{s}, with
...
\item The analysis of the number of iterations \cite{scev} provides a \scev{}
  ...
  \scev{s} of all the variables involved in each condition that we translate.
...
We do not yet have a heuristic on how to limit the \SCoP{s} to only those
...
\SCoP{} detection still tries to maximize the size of \SCoP{s}, with the
...
\SCoP{s}. While adding a new \SCoP{} to the set of detected \SCoP{s}, we first
...
\SCoP{s} maintained in the set are mutually exclusive w.r.t{.}{~}the regions they
span i.e., no \SCoP{} intersects with another in the set of detected \SCoP{s}.
...
\SESE{} in their respective sub-\SCoP{s}, we need to re-analyze them because the
...
saved, after removing any intersecting or sub-\SCoP{s}, and further analysis is
continued to extend the \SCoP{} again.  If no such \SESE{} could be found, the
algorithm keeps them as two separate \SCoP{s}, and continues by trying to extend
the second \SCoP{.}
...
\SESE{} which subsumes both the \SCoP{s}, otherwise it returns an invalid
...
\SCoP{s} with only one loop.  The improved algorithm for \SCoP{} detection does
allow for the discovery of \SCoP{s} with single loops, which could be profitably
...
all \SCoP{s} with less than $2$ loops, a choice we will revisit when we will
have a reason to enable single loop \SCoP{s}.
...
It may appear to the reader that the initial \SCoP{} detection overhead was low
enough for industrial uses, i.e., less than $2\%$ of the total compilation time.
However, when looking at Table-\ref{tab:overall-insns}, we see that cumulatively
over the large codebase of the source code of \GCC{,} the improvement of the new
\SCoP{} detection results in an order of magnitude reduction: $1.5e^{10}$
vs{.}{~}$6.7e^8$ number of instructions used for \SCoP{} detection.  In essence,
in order to enable polyhedral compilation by default in an industrial compiler,
the \SCoP{} detection should be as neutral as possible, as it is the only part
of a polyhedral compiler that will execute on all codes.
