...
executed in the new and old SCoP detection on several benchmarks: Tramp3d-v4
\cite{richi} is a large C++ application, Polybench, and all the source files of
GCC 6.0.  On large applications, the speedup over the old algorithm for scop
detection is very important: on Tramp3d-v4, the old SCoP detection accounted for
$7\%$ of the compilation time, while the new SCoP detection now only takes
$.33\%$.  On Polybench there is some slowdown corresponding to the detection of
larger scops: every step in enlarging the scop may trigger extra computations
for scevs.  The slowdowns only correspond to benchmarks with loops that can be
optimized by a polyhedral compilation.
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|}
      ...
      GCC 6.0    & $1.5e^{10}$ & $6.7e^8$ & $22.6$  & $6.2e^{12}$ & $0.24$ & $0.01$  \\
% Polybench: old=333775317, new=475127633, main=
% Tramp3d-v4: old=1755147630, new=624729992, main=
% GCC: old=15072748886, new=667787862, main=6183888029882
      \hline
    \end{tabular}}
  \end{center}
...
\begin {figure}
  \begin{center}
    \resizebox{.95\linewidth}{!}{\input{polybench/speedup}}
  \end{center}
...
\begin {figure}
  \begin{center}
    \resizebox{.95\linewidth}{!}{\input{gcc/speedup}}
  \end{center}
